...
export default function ProfilePage() {
  ...
  const handleAvatarUpload = async (
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/profile/updateUser`,
        {
          method: "POST",
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/profile/deleteUser`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
  ...
  return (
    <div className="space-y-6">
        ...
        <div key={key}>
          ...
          <input
            ...
            className={`w-full p-2 mt-2 rounded border ${
              darkMode &&
              "bg-[#1a1e1f] shadow-lg shadow-white/10 border-[#2D2D2D]"
            }`}
            placeholder={label}
          />
        </div>
      ...
      <div>
        <span className="text-[#777C90]">
          ...
          <span
            ...
            className={`${
              darkMode
                ? "text-[#f8f8f8] hover:text-[#f0f0f2]"
                : "text-[#2D2D2D]"
            } hover:text-[#0F2528] cursor-pointer`}
          >
            Reset password
          </span>
        </span>
      ...
      <div className="flex gap-4">
        ...
        <button className="bg-red-500 text-white p-2 rounded" onClick={handleDeleteAccount}>
          Delete Account
        </button>
      </div>
