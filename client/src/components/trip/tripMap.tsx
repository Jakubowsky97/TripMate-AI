  ...
  useEffect(() => {
    ...
    loader.load().then(() => {
      ...
      <div id="info-window-${place.name}">
        ...
        <button style="margin-top: 8px; background-color: #ef4444; color: white; padding: 4px 8px; border: none; border-radius: 4px; cursor: pointer;" id="remove-marker-btn-${place.name}">
          Usuń miejsce
        </button>
      </div>
    `;
